...
class StudentBottomNavInstrumentedTest {
        ...
        muSetContent(composeRule, AppRole.Student) {
            ...
            NavHost(navController = nav, startDestination = Screen.StudentHome.route) {
                ...
                composable(Screen.Schedule.route) {
                    Text("STUB_SCHEDULE_ROUTE")
                }
            }
        }
        composeRule.waitUntil(timeoutMillis = 20_000) {
            runCatching { composeRule.onNodeWithTag(UiTestTags.Screen.STUDENT_HOME).fetchSemanticsNode(); true }
                .getOrDefault(false)
        }
        composeRule.onNodeWithTag(UiTestTags.BottomNav.STUDENT_SCHEDULE).performClick()
        composeRule.waitUntil(timeoutMillis = 15_000) {
            runCatching { composeRule.onNodeWithText("STUB_SCHEDULE_ROUTE").fetchSemanticsNode(); true }
                .getOrDefault(false)
        }
        composeRule.onNodeWithText("STUB_SCHEDULE_ROUTE").assertIsDisplayed()
    }
}
